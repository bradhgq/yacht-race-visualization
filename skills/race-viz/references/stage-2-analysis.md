...
- **VMC (velocity made good on course)** — closing speed on the finish, a centered difference of the gridded distance-to-finish (`(dtf[i-1] − dtf[i+1]) / (t[i+1] − t[i-1])`), smoothed on the same ~30-min window as SOG. Negatives are real (tacks; sailing away from the mark in light air) — ship them. Validate that the official-window mean recovers `rhumb ÷ elapsed` (∫VMC dt = course) and freeze it as a golden. **Terminology gate (I18): call it VMC, never VMG** — a tracker carries no wind, so wind-referenced VMG is impossible; say so in the caption. **Prerequisite:** VMC is only as good as the distance-remaining function — a straight rhumb uses great-circle-to-finish, but **mark courses (Block Island, ALIR) need routed distance-remaining (`pipeline/route.py`)** first.
