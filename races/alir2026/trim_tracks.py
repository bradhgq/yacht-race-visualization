...
Run from the repo root:  python3 races/alir2026/trim_tracks.py
"""
import csv
import pathlib
import sys
...
def main():
    ...
    src = RACE / "raw" / "alir2026_tracks.csv"
    dst = RACE / "raw" / "alir2026_tracks_clean.csv"

    windows = race_windows(RACE / "raw" / "alir2026_results.csv", cfg["time"]["utc_offset"])
    ...
    for boat, pings in by_boat.items():
        ...
        if fin is not None:
            ...
            # travels home TOWARD the finish keeps "approaching" it (2025's Ricky
            # Bobby anchored 12 h off Mattituck, then rode down the LIE at ~60 kt,
            ...
            #       after retiring,
            #   (c) the ping before the first SUSTAINED > VEHICLE_KT run —
            #       a car or a tow, not a boat (fleet max under sail: 13.8 kt).
            #
            # Two 2026 adjustments, both found when the 2025 rules misfired on
            # this year's data (stage-0 record, trim_adjustments):
            #   - Scan only pings at/after start_cut. Elizabeth (RET) carried a
            #     pre-start GPS spike; scanning from the dock cut her END before
            #     her START and dropped all 967 pings of a boat that raced.
            #   - The vehicle rule needs THREE consecutive over-threshold
            #     intervals. 2026's feed has a ~1-min cadence tier (40% of
            #     intervals; 2025 was uniformly 5-min), and one-minute GPS
            #     jitter reads as 26-91 kt for a single interval. A glitch
            #     produces at most two fast intervals (out and back); a real
            #     vehicle is fast for many. Five still-racing boats were being
            #     cut mid-race by single-interval spikes.
            racing = [r for r in uniq if int(r["epoch"]) >= start_cut]
            if not racing:
                report.append((boat, len(pings), 0, dups, "no pings after start_cut"))
                continue
            lat = [float(r["lat"]) for r in racing]
            lon = [float(r["lon"]) for r in racing]
            eps = [int(r["epoch"]) for r in racing]
            dtf, _xte, _leg = course.dtf_xte(lat, lon)
            cands = [(min(range(len(racing)), key=lambda i: dtf[i]), "furthest point")]
            fast = [False] * len(racing)
            for i in range(1, len(racing)):
                dt = eps[i] - eps[i - 1]
                d = geo_nm(lat[i - 1], lon[i - 1], lat[i], lon[i])
                fast[i] = dt > 0 and d / (dt / 3600) > VEHICLE_KT
            for i in range(1, len(racing) - 2):
                if fast[i] and fast[i + 1] and fast[i + 2]:
                    cands.append((i - 1, "sustained vehicle speed next"))
                    break
            for i in range(len(racing)):
                j = i
                while j + 1 < len(racing) and geo_nm(lat[i], lon[i], lat[j + 1], lon[j + 1]) < STATIONARY_NM:
                    j += 1
