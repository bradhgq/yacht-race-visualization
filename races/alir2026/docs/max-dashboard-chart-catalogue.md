...
**BUILT AND SHIPPING IN THE PRIVATE CUT (2026-07-27)**: C1+C2 → `maxwind.js` ·
C7+C8+C9+C10 → `maxfcst.js` · C14+C15 → `maxpolar.js` · C15+C16 → `maxsails.js` ·
C29 → `maxtruth.js`. All five render from `modules/maxdata.js`, generated by
`scripts/gen_max_constants.py` — the evidence-constants pattern (`forecast.js`
precedent), which is why `out/dashboard_data.json` and therefore the snapshot
compare (I16) are untouched by this whole layer. The remaining 21 entries are
proposals.
...
### C1 · The measured-wind ribbon  ✅ BUILT → `maxwind.js`
...
### C2 · The dawn-park compass — the signature chart  ✅ BUILT → `maxwind.js`
...
### C7 · The onboard forecast report card  ✅ BUILT → `maxfcst.js`
...
### C8 · "The park was not forecast"  ✅ BUILT → `maxfcst.js`
...
### C9 · The 162° miss  ✅ BUILT → `maxfcst.js`
...
### C10 · Information age — the staleness clock  ✅ BUILT → `maxfcst.js`
...
### C14 · The report card  ✅ BUILT → `maxpolar.js`
...
### C15 · The downwind signature — the crew-facing headline  ✅ BUILT → `maxpolar.js + maxsails.js`
...
### C16 · The matched-TWS control  ✅ BUILT → `maxsails.js`
...
### C29 · The corroboration panel  ✅ BUILT → `maxtruth.js`
