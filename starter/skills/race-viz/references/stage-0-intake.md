...
No files yet? If the user supplies identifiers instead (a YachtScoring event id or a YB race id),
read `references/data-acquisition.md` and emit the acquisition handoff first — its outputs are
exactly this manifest's required trio.
...
4. **Course model.** Point-to-point → great-circle distance-to-finish spine. Marks → routed distance-remaining along the ordered course line (`pipeline/route.py` in the starter repo; escalate to Claude Code only for geometry it can't express, e.g. multi-lap courses). Record the official course length from race documents when available, rather than a computed guess.
