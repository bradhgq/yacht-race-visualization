...
> **v1.2 note (2026-07-08) — repo consolidated.** The starter became the public
> monorepo `bradhgq/yacht-race-visualization`. Current layout supersedes the
> `## Layout` block below: `starter/` holds engine code only (shell, pipeline,
> adapters, acquisition, tests); `skills/race-viz/`, `docs/`, and `races/` are
> now **top-level**; the worked example is `races/nb2026/` (a real directory —
> the old `races/nb2026 → examples/nb2026` symlink and `examples/` are retired),
> with its deployed build force-committed at `races/nb2026/dist/`. Build/harness
> invoke from the repo root (`python3 starter/shell/build.py races/<race>`).
> A full v2 rewrite folding in the RETROSPECTIVE_ROUND2 amendment list (I15
> finstrip height, `bindMode` scope arg, `speed`/`controls`/module self-scopes,
> the one-command chain wrapper, routed-DTF closed, distspeed promoted,
> divisions/ratingBands schema) is still pending; the round-2/3 deltas are
> captured in `races/nb2026/decisions/R2-*.yaml` and `R3-*.yaml` meanwhile.
