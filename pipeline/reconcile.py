...
def reconcile(fixes, client_series, utc_offset, matched_key='matched_local'):
    ...
    (the worked example ships it as 'matched_edt').
    """
    rows = []
    for fx in fixes:
        ...
        rows.append({'t': fx['label'], matched_key: fx['t_local'][11:], 'temp': fx['temp'],
                     ...
                     'log': [round(la, 3), round(lo, 3)],
                     'trk': [round(float(r['lat']), 3), round(float(r['lon']), 3)],
                     'd': round(dnm, 1), 'verdict': fx['verdict'], 'note': fx['note']})
    return rows
